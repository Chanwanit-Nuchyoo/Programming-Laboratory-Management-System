<!-- application/controllers/Supervisor.php -->
<?php
...
class Supervisor extends MY_Controller {
	...
	*/	
	...
	public function student_show($stu_group_id) {
		...
		*/
		...
		$midterm_scores = $this->lab_model->get_midterm_score($stu_group_id);
		$this->show_to_console($midterm_scores);
	...
	public function group_management() {
		$_SESSION['selected_menu']='grp_management';
		$this->update_last_seen();
		//echo "<h2>". __METHOD__ ." : _SESSION :</h2><pre>"; print_r($_SESSION); echo "</pre>";
		//echo "<h3>". __METHOD__ ." : _POST : size = ". sizeof($_POST)."</h3><pre>"; print_r($_POST); echo "</pre>";
		$user_id = $_SESSION['id'];
		...
		$this->load->model('lab_model');
		$supervised_groups = $this->lab_model->get_supervise_group($user_id,2022,1); // return array of group_id
		...
		$supervised_groups = $this->lab_model->get_supervise_group($user_id,2022,2); // return array of group_id
		$groups = array_merge($supervised_groups,$groups);
		sort($groups);
		...
		// echo "<!-- group_list <pre>"; print_r($group_list); echo "</pre> -->";exit();
	...
	public function select_exercise_for_group() {
		...
		// asort($lab_list);
		// echo "<pre>"; print_r($lab_list); echo "</pre>";
	...
	public function student_exercise_view($stu_id,$chapter,$item) {		
		...
		$lab_exercise = $this->lab_model->get_lab_exercise_by_id($exercise_id);
		$this->show_to_console($lab_exercise);
		if (isset($lab_exercise['testcase'])) {
			$testcases = $lab_exercise['testcase'];
		
			if (is_array($testcases) && $lab_exercise['testcase'] != 'no_input') {
				$number_of_testcase = count($testcases);
			} else {
		...
		// echo "<h2 style='color:pink'>lab_exercise = <pre>";print_r($lab_exercise);echo "</pre></h2>"; 
		// echo "<h2 style='color:blue'>lab_exercise testcase = <br><pre>";print_r($lab_exercise['testcase']);echo "</pre></h2>"; 		
		// echo "number of testcase : $number_of_testcase <br>";
		// exit();
		...
		//echo "<h2 style='color:orange;'>stu_submit = <pre>";print_r($stu_submit);echo "</pre></h2>";  
		$data_for_testcase = $this->get_data_for_testcase($stu_id,$exercise_id);
		//echo "<h2 style='color:blue;'>data_for_testcase = <pre>";print_r($data_for_testcase);echo "</pre></h2>";  
		$last_submit = null;
		if (isset($data_for_testcase['last_submit']))
			$last_submit = $data_for_testcase['last_submit'];
		//echo "<h2 style='color:pink;'>last_submit = <pre>";print_r($last_submit);echo "</pre></h2>";  

		$this->load->model('student_model');
		$student_data = $this->student_model->retrieve_student_record($stu_id);
		//echo "<h2 style='color:red;'>student_data = <pre>";print_r($student_data);echo "</pre></h2>";  //exit();
		if (isset($lab_exercise['testcase'])) {
			//$lab_exercise['testcase']
		} else {
			$lab_exercise['testcase']='no_input';
		}
		if ($lab_exercise['testcase']=='no_input') {
			$number_of_testcase = 0;
		...
		//echo "<h2 style='color:red;'data_ = <pre>";print_r($data_);echo "</pre></h2>";  //exit();	
		$data_for_student_view = $this->get_data_for_student_view($stu_id,$chapter,$item);
		//echo "<h2 style='color:red;'>student_data = <pre>";print_r($data_for_student_view);echo "</pre></h2>";  //exit();
		//echo "number of testcase : $number_of_testcase <br>";
	...
	*/
	...
	// 2565-10-04 kanut
	public function upload_midterm_score()	{
		$_SESSION['selected_menu'] ='score';
		$this->load->view('supervisor/head');
		$this->load->view('supervisor/nav_fixtop');
		$this->load->view('supervisor/nav_sideleft');
		$this->load->view('supervisor/upload_midterm_score');		
		$this->load->view('supervisor/footer');
	}//public function practice_exam()

	// 2565-10-04 kanut
	public function upload_midterm_score_action()	{
		// echo "<pre>"; print_r($_POST); echo "</pre>";
		$scores = $_POST['scores'];
		$s_arr = explode("\n",$scores);
		foreach ($s_arr as $line ) {
			...
			$id = (int) $b;
			$score = (int) $c;
			echo "id=$id ".gettype($id)." score=$score ".gettype($score)."<br>";
			$this->load->model('supervisor_model');
			$this->supervisor_model->update_midscore($id,$score);
		} 
	}//public function practice_exam()
	
	// 2595-10-20
	public function logoutStudentGroup($group_id)
	{
		// echo json_encode("Not permitted.");
		// 	return ;
		if (intval($_SESSION['ci_session'])<=0 || $_SESSION['role']!='supervisor') {
			// echo json_encode("Not permitted.");
			return ;
		}
		$this->load->model('lab_model');
		$stu_list = $this->lab_model->get_students_by_group_id($group_id);
		...
		foreach( $stu_list as $stu) {
		   if( intval( $stu['ci_session'] ) > 0 && $stu['status']=="online") {
			   ...
			   $this->lab_model->logout_student($stu_id);
			   $count++;
			   array_push($stu_logout,$stu);
		   }
		}
		echo json_encode($stu_logout);
	}

	// 2565-11-03 kanut
	public function check_score()	{
		$_SESSION['selected_menu'] ='check_score';
		$this->load->model('lab_model');
		$quizes = $this->lab_model->get_quizes();				
		$_SESSION['quizes'] = $quizes;			
		$this->load->view('supervisor/check_score');	
	}//public function 
	// 2565-11-03 kanut
	public function check_score1()	{
		$_SESSION['selected_menu'] ='check_score';
		$this->load->model('lab_model');
		$quizes = $this->lab_model->get_quizes();				
		$_SESSION['quizes'] = $quizes;			
		$this->load->view('supervisor/check_score1');	
	}//public function 
