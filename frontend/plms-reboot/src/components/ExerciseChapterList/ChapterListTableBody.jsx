...
const renderScoreBox = (groupId, item, index, onStudentPage, isAccessible, insPage, getUrl) => {
    ...
    <Stack direction="row" spacing="5px" sx={{
      ...
      <Box width={395} className="row-info-box">
        <Stack direction="row" flexWrap="wrap">
          {chapter?.items && chapter.items.map((item, index) => renderScoreBox(chapter.class_id, item, index, onStudentPage, isAccessible, insPage, getUrl))}
        </Stack>
      </Box>
...
export default memo(ChapterListTableBody)